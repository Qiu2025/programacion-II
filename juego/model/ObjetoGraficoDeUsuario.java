...
import proyectoPrueba.Proyecto;
import java.awt.event.KeyEvent;
import proyectoPrueba.model.BalaHeroe;
...
public abstract class ObjetoGraficoDeUsuario extends ObjetoGrafico {
    ...
    public void eventoUsuarioTeclaMover(){
        ...
        // Usamos java.util (KeyEvent) para comprobar teclas porque el movimiento es más fluido que StdDraw (nextKeyTyped)
            // Subir
        ...
        // Comprobar si el usuario ha presionado el teclado (este if es necesario para que StdDraw.nextKeyTyped no dé error)
        if(StdDraw.hasNextKeyTyped()){
            // Guardamos StdDraw.nextKeyTyped en un char para luego realizar la comprobación porque
            // por algun motivo ejecutar dos veces StdDdraw.nextKeyTyped da error:
            // Exception in thread "main" java.util.NoSuchElementException: your program has already processed all keystrokes
            ...
            if(key == 'p'  && !pausado){
                ...
                // y además se ha presionado la tecla 1 (if de arriba), habilidad 1 pasa a estar activada
                if(!habilidad1Activada && System.currentTimeMillis() > momentoActivacionHabilidad1 + duracionHabilidad1 + cdHabilidad1 ){
                ...
                // y además se ha presionado la tecla 2 (if de arriba), habilidad 2 pasa a estar activa 
                if(!habilidad2Activada && System.currentTimeMillis() > momentoActivacionHabilidad2 + duracionHabilidad2 + cdHabilidad2){
        ...
        // Si la habilidad 1 está activa y ha pasado el tiempo de duración, la desactivamos
        // (efecto de habilidad 1 es mantener al heroe inmune de ataques, implementada en Proyecto -> comprobarColisiones)
        ...
        if(habilidad2Activada){
            // si ha pasado el tiempo de duración, desactivamos la habilidad y cambiamos el radio de la bala a la inicial

...
public class Proyecto extends Juego2DBase {
    ...
    // variables que guardan temporalmente el tiempo actual para conseguir un
    // retardo antes de la primera aparición de los objetos
    ...
    protected void pintarObjetos() {
        ...
        if (ObjetoGraficoDeUsuario.habilidad1Activada())
            StdDraw.picture(heroe.getFigura().getCentroide().getX(), heroe.getFigura().getCentroide().getY(),
                    "../Assets/Escudo.png", heroe.getFigura().getWidth(), heroe.getFigura().getWidth());

        // Pintar barra de vida del heroe
        StdDraw.setPenColor(StdDraw.RED);
        StdDraw.setPenRadius(0.01);
        StdDraw.filledRectangle(20 + 30 * heroe.getVida() / Heroe.getVidaInicial(), 95,
                30 * heroe.getVida() / Heroe.getVidaInicial(), 3);
        StdDraw.setPenColor(StdDraw.BLACK);
        ...
        StdDraw.picture(5, 15, "../Assets/Escudo.png", 5, 5);
        StdDraw.setPenColor(StdDraw.BLACK);
        ...
        StdDraw.picture(5, 5, "../Assets/Bullet.png", 7, 7);
        StdDraw.setPenColor(StdDraw.BLACK);
    ...
    protected void comprobarColisiones() {
        for (int i = 0; i < balasEnemigo.size(); i++) {
            ...
            if (balasEnemigo.get(i).getFigura().getCentroide().getX() < 0
                    || balasEnemigo.get(i).getFigura().getCentroide().getX() > 100
                    || balasEnemigo.get(i).getFigura().getCentroide().getY() < 0
                    || balasEnemigo.get(i).getFigura().getCentroide().getY() > 100) {
                balasEnemigo.remove(i);
                // Si la bala colisiona con el heroe, se borra y baja la vida del heroe si
                // habilidad 1 no está activa
        ...
        for (int i = balasHeroe.size() - 1; i >= 0; i--) {
            ...
            if (balasHeroe.get(i).getFigura().getCentroide().getX() < 0
                    || balasHeroe.get(i).getFigura().getCentroide().getX() > 100
                    || balasHeroe.get(i).getFigura().getCentroide().getY() < 0
                    || balasHeroe.get(i).getFigura().getCentroide().getY() > 100) {
                balasHeroe.remove(i);

                // Si la bala de heroe colisiona con algun enemigo, borrar la bala y bajar vida
                // del enemigo colisionado
            } else {
    ...
    protected void moverObjetos() {
        // Si el usuario presiona una tecla comprobar si es una de las que tiene
        // funcionalidades (WASD y P), si lo es realizar
        // la accion correspondiente. Luego comprobar si el heroe tras la acción se sale
        // del canvas, en cuyo caso moverlo a la
        // esquina contraria
    ...
    protected void crearObjetos() {
        ...
        // crear enemigos si ha pasado el "retardo de aparicion del primer enemigo"
        // establecido (tiempoAparicionEnemigo)
        ...
        for (int i = 0; i < enemigos.size(); i++) {
            if (System.currentTimeMillis() > enemigos.get(i).getTiempoAtaqueEnemigo()
                    + enemigos.get(i).getFrecAtaque()) {
